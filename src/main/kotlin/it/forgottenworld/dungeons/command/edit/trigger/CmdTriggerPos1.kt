...
fun cmdTriggerPos1(sender: CommandSender, command: Command, label: String, args: Array<String>): Boolean {
    ...
    DungeonEditManager.wipTriggerPos2s[sender.uniqueId]?.let { p2 ->
        ...
        box.highlightAll()
        DungeonEditManager.wipTriggerPos2s.remove(sender.uniqueId)
        sender.sendFWDMessage("Created trigger with id $id")
    } ?: run {
        DungeonEditManager.wipTriggerPos1s[sender.uniqueId] = block
