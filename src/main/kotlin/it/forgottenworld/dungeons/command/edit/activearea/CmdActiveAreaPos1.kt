...
fun cmdActiveAreaPos1(sender: CommandSender, command: Command, label: String, args: Array<String>): Boolean {
    ...
    DungeonEditState.wipActiveAreaPos2s[sender.uniqueId]?.let { p2 ->
        ...
        box.highlightAll()
        DungeonEditState.wipActiveAreaPos2s.remove(sender.uniqueId)
        sender.sendFWDMessage("Created active area with id $id")
    } ?: run {
        DungeonEditState.wipActiveAreaPos1s[sender.uniqueId] = block
